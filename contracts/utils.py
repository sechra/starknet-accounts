...
async def print_n_wait(client: Client, invocation: InvokeFunction):
    print(f"\u001b[35mTransaction Hash: {invocation.hash}\u001b[0m\n")
    ...
    if "ACCEPT" in str(res.status):
        print(f"\u001b[32;1mTx Results: {res.status}\u001b[0m")
        ...
        print(f"\u001b[31;1mTx Results: {res.status}")
...
async def deploy_testnet(
    ...
    data = dict()
    CONTRACT_ADDRESS = f"{contract_path.upper()}_ADDRESS"
    if additional_data:
        CONTRACT_ADDRESS = f"{CONTRACT_ADDRESS}_{additional_data}"
        ...
        if os.path.exists(ACCOUNT_FILE) and os.path.getsize(ACCOUNT_FILE) > 0:
            with open(ACCOUNT_FILE) as json_file:
                ...
                if CONTRACT_ADDRESS in data:
                    print(
                        f"\u001b[35mFound local contract: {data[CONTRACT_ADDRESS]}\u001b[0m\n"
                    )
    ...
    os.system(
        f"starknet-compile --account_contract {contract_path}.cairo --output {contract_path}_compiled.json"
    )

    compiled = Path(f"{contract_path}_compiled.json").read_text()
    deployment_result = await Contract.deploy(
        client, compiled_contract=compiled, constructor_args=constructor_args
    )
    os.system(f"rm {contract_path}_compiled.json")
    ...
    if os.getenv("ACCOUNT_CACHE") != "false":
        data[CONTRACT_ADDRESS] = f"0x{res.transaction.contract_address:02x}"
...
async def compile_deploy(client, contract="", args=[], salt=0):
    ...
    compiled = f"{contract}_compiled.json"
    os.system(f"starknet-compile {contract}.cairo --output {compiled}")
    deployment_result = await Contract.deploy(
        client=client,
...
async def devnet_funding(data, toAddr):
    acc_client, _ = devnet_account(data)
